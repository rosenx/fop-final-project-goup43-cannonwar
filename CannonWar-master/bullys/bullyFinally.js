 ...
 * 反弹子弹类
 */
class BouncyBullet extends Bully {
    constructor(pos, speed, father, damage, r) {
        super(pos, speed, father, damage, r);
        this.bounceCount = 3; // 最大反弹次数
    }

    collide(world) {
        let c = new Circle(this.pos.x, this.pos.y, this.r);
        let collided = false;
        ...
        if (collided) {
            ...
            if (this.bounceCount <= 0) {
                ...
                // 改变子弹方向，模拟反弹
                this.speed = this.speed.neg(); // 简单实现方向反转
            }
        }
    }
}
 ...
 * 水花子弹类
 */
class SplashBullet extends Bully {
    constructor(pos, speed, father, damage, r) {
        super(pos, speed, father, damage, r);
        this.slowEffect = 0.5; // 减速倍率
    }
    ...
    collide(world) {
        ...
        for (let m of world.monsters) {
            if (c.impact(m.getBodyCircle())) {
                m.hpChange(-this.damage);
                m.speedFreezeNumb *= this.slowEffect; // 降低速度

                // 添加水花特效
                let e = new EffectCircle(this.pos.copy());
                e.circle.r = this.r * 2;
                e.animationFunc = e.splashBlueAnimation;
                e.duration = 30; // 动画持续时间
                world.addEffect(e);
 ...
 * 闪电子弹类
 */
class LightningBullet extends Bully {
    constructor(pos, speed, father, damage, r) {
        super(pos, speed, father, damage, r);
        this.chainRadius = 50; // 闪电传播范围
        this.chainDamage = damage * 0.5; // 连锁伤害
    }
    ...
    collide(world) {
        ...
        for (let m of world.monsters) {
            if (c.impact(m.getBodyCircle())) {
                ...
                // 连锁攻击
                for (let other of world.monsters) {
                    if (other !== m && other.pos.dis(m.pos) <= this.chainRadius) {
                        other.hpChange(-this.chainDamage);

                        // 添加闪电特效
                        let e = new EffectLine(m.pos.copy(), other.pos.copy());
                        e.animationFunc = e.flashElectricAnimation;
                        e.duration = 20; // 动画持续时间
                        world.addEffect(e);
                    }
                }
...
const BULLET_TYPES = {
    normal: Bully,         // 普通子弹
    bouncy: BouncyBullet,  // 反弹子弹
    splash: SplashBullet,  // 水花子弹
    lightning: LightningBullet, // 闪电子弹
};

/**
 * 子弹生成示例
 * @param type {string} 子弹类型
 * @param pos {Vector} 子弹位置
 * @param speed {Vector} 子弹速度
 * @param father {Tower} 发射子弹的塔
 * @param damage {number} 子弹伤害
 * @param r {number} 子弹半径
 */
function createBullet(type, pos, speed, father, damage, r) {
    let BulletClass = BULLET_TYPES[type] || BULLET_TYPES['normal'];
    return new BulletClass(pos, speed, father, damage, r);
}
